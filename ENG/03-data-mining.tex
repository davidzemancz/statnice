...
community detection and link prediction (NAIL116). The most conspicuous gaps are the
\textbf{KDD process and methodologies} (Chapter~\ref{sec:paradigmata}) and
\textbf{characteristic and discriminant rules} (Chapter~\ref{sec:pravidla}): the topic names
them explicitly, but \emph{neither course covers them} --- they are therefore treated here
following Han--Kamber.
\end{poznbox}
\end{minipage}
\end{center}
...
\mimo{} The whole chapter is beyond the slides of both courses --- but the topic names it
explicitly and it is standard material, treated here following Han--Kamber. For the connection
with generalisation in OLAP see Chapter~\ref{sec:paradigmata}.

\subsection{Characterisation vs.\ discrimination}

Descriptive data mining answers two related but different questions:

\begin{defbox}[Characterisation vs.\ discrimination]
\textbf{Data characterisation} --- a summary of the general properties of the \emph{target class}.
It answers the question: \uv{What does a typical client with a mortgage look like?} The result
consists of \emph{characteristic rules}.

\textbf{Data discrimination} --- a comparison of the general properties of the target class with
those of one or more \emph{contrasting classes}. It answers the question: \uv{How does a client
with a mortgage differ from a client without one?} The result consists of \emph{discriminant
rules}.

The difference is fundamental: a property can be \emph{typical} of the target class (a high
proportion inside the class) and still have no \emph{discriminating} power, because it is equally
typical of the contrasting class.
\end{defbox}

Both tasks are solved by \emph{attribute-oriented induction} (AOI), proposed by Han, Cai and
Cercone (1991). An alternative is the \emph{data cube} approach known from OLAP: the \emph{roll-up}
operation corresponds to generalisation, \emph{drill-down} to specialisation. AOI is more flexible
(it handles attributes without a pre-materialised cube), OLAP is faster (pre-computed aggregations).

\subsection{Concept hierarchies}

\begin{defbox}[Concept hierarchy]
A \emph{concept hierarchy} of an attribute $A$ is a partial ordering of its values from the most
concrete (the leaves) to the most general (the root \texttt{ANY}). Types:
\begin{itemize}
\item \emph{schema hierarchy} --- given by the structure of the data: street $\prec$ city $\prec$
      region $\prec$ country;
\item \emph{set-grouping} --- an explicit grouping of values: \{20--29\} $\prec$ \emph{young};
\item \emph{operation-derived} --- derived by a computation: e-mail address $\to$ domain;
\item \emph{rule-based} --- defined by rules over several attributes.
\end{itemize}
The hierarchy is supplied by an expert, or generated automatically (by discretisation for numeric
attributes, and by the number of distinct values for categorical ones --- an attribute with more
distinct values is usually at a lower level of the hierarchy).
\end{defbox}

\subsection{Attribute-oriented induction (AOI)}

\begin{defbox}[The AOI algorithm]
\textbf{Input:} the database, a query delimiting the target class, a list of attributes,
generalisation thresholds $T_i$ for the individual attributes and an overall threshold $T$ for the
size of the resulting relation.

\begin{algorithmic}[1]
\State \textbf{data collection:} select by the query the set of objects of the target class (the
       \emph{initial working relation})
\State add to each object the attribute \texttt{count} $=1$
\For{each attribute $A_i$}
   \If{$A_i$ has many distinct values and \emph{no} concept hierarchy exists for it}
      \State \textbf{attribute removal}
   \ElsIf{$A_i$ has many distinct values but a hierarchy exists}
      \State \textbf{attribute generalisation}: replace the values by their ancestors one level up,
      \State until the number of distinct values is $\le T_i$
   \EndIf
\EndFor
\State \textbf{aggregation:} merge identical generalised objects and add up their \texttt{count}
\While{the number of rows of the resulting relation $>T$}
   \State generalise further the attribute whose generalisation reduces the number of rows the most
\EndWhile
\State \textbf{presentation:} display the \emph{prime relation} as a table, a cross-tabulation, a
       chart,
\State or convert it into quantitative rules
\end{algorithmic}
\end{defbox}

\textbf{The two basic generalisation operations:}
\begin{itemize}
\item \textbf{Attribute removal} --- if an attribute has too many distinct values and (a)~no
      generalisation operator exists for it, or (b)~its higher levels are expressed by other
      attributes, the attribute is removed from the relation. Examples: \emph{name},
      \emph{telephone}, \emph{personal identification number}.
\item \textbf{Attribute generalisation} --- if a generalisation operator (a concept hierarchy)
      exists, the values are replaced by values at a higher level. Examples: \emph{date of birth}
      $\to$ \emph{age} $\to$ \emph{age category}; \emph{city} $\to$ \emph{region} $\to$
      \emph{macro-region}.
\end{itemize}

\textbf{Control of the generalisation} is exercised through two thresholds: the
\emph{attribute generalisation threshold} $T_i$ --- the maximum permitted number of distinct values
of the attribute (typically 2--8); and the \emph{relation size threshold} $T$ --- the maximum
number of rows of the resulting prime relation (typically 10--30). Too much generalisation leads to
trivial results (\uv{all clients are humans}), too little to an unreadable table. A suitable level
is found interactively (drill-down / roll-up).

The complexity is $O(n\,a)$ for $n$ objects and $a$ attributes (one pass over the data plus
aggregation), provided the generalisation is realised through a hash table of generalised records
--- AOI is therefore \emph{linear} in the number of objects, which is essential for large
databases.

\subsection{The t-weight and d-weight measures}

\begin{defbox}[t-weight and d-weight]
Let the prime relation contain the \emph{generalised tuples} $q_1,\dots,q_N$.

The \textbf{typicality} (t-weight) of the tuple $q_a$ in the target class:
\[ t\text{-}\mathrm{weight}(q_a)=\frac{\mathrm{count}(q_a)}{\sum_{i=1}^{N}\mathrm{count}(q_i)}
   \in[0,1], \]
i.e.\ the fraction of the target-class objects that the tuple $q_a$ covers. The sum of the
t-weights over all tuples of the prime relation is 1 (100~\%). It serves for
\emph{characteristic} rules.

The \textbf{discriminating weight} (d-weight) of the tuple $q_a$ with respect to the target class
$C_{target}$ and the contrasting classes $C_1,\dots,C_m$:
...
i.e.\ the fraction of the objects covered by $q_a$ that belong to the target class. It serves for
\emph{discriminant} rules. A high d-weight ($\to 1$) means that the tuple strongly distinguishes
the target class; a value close to the class proportion means no discriminating ability at all.
\end{defbox}

\begin{defbox}[Quantitative characteristic, discriminant and description rule]
A \textbf{quantitative characteristic rule} (a necessary condition --- an implication \emph{from}
the class):
\[ \forall X\ \ \mathrm{target\_class}(X)\Rightarrow \mathrm{cond}_1(X)\,[t{:}\,w_1]\ \vee\
   \cdots\ \vee\ \mathrm{cond}_N(X)\,[t{:}\,w_N] \]
Read: if $X$ belongs to the target class, then it satisfies $\mathrm{cond}_1$ with typicality
$w_1$, or $\mathrm{cond}_2$ with typicality $w_2$, \dots

A \textbf{quantitative discriminant rule} (a sufficient condition --- an implication \emph{into}
the class):
\[ \forall X\ \ \mathrm{target\_class}(X)\Leftarrow \mathrm{cond}(X)\,[d{:}\,w] \]
Read: if $X$ satisfies the condition $\mathrm{cond}$, it belongs to the target class with
probability $w$.

A \textbf{quantitative description rule} combines both (a necessary and sufficient condition):
\[ \forall X\ \ \mathrm{target\_class}(X)\Leftrightarrow \mathrm{cond}_1(X)\,
   [t{:}\,w_1,\ d{:}\,w_1']\ \vee\ \cdots\ \vee\ \mathrm{cond}_N(X)\,[t{:}\,w_N,\ d{:}\,w_N'] \]
\end{defbox}

\begin{defbox}[Example --- characterisation and discrimination of bank clients]
\textbf{Target class:} clients with a mortgage (200 persons). \textbf{Contrasting class:} clients
without a mortgage (800 persons).

After AOI (\emph{name}, \emph{personal identification number} and \emph{telephone} removed;
\emph{date of birth} $\to$ \emph{age category}, \emph{salary} $\to$ \emph{income category},
\emph{municipality} $\to$ \emph{region} generalised) the prime relation is:
...
\midrule
\multicolumn{3}{@{}l}{Total} & 200 & 800 & 100~\% & \\
\bottomrule
\end{tabular}
\end{center}

\textbf{The computation.} The $t\text{-}\mathrm{weight}$ of the first tuple is $90/200=45\,\%$; its
$d\text{-}\mathrm{weight}$ is $90/(90+10)=90\,\%$. For the second tuple $t=50/200=25\,\%$,
$d=50/(50+150)=25\,\%$. For the fourth $t=20/200=10\,\%$, $d=20/(20+480)=4\,\%$.

\textbf{The quantitative characteristic rule:}
\begin{align*}
\forall X\ \ \text{mortgage}(X)\Rightarrow\ & \big(\text{age}(X){=}\text{30--40}\wedge
  \text{income}(X){=}\text{high}\wedge\text{region}(X){=}\text{Prague}\big)\ [t{:}\,45\,\%]\\
\vee\ & \big(\text{age}(X){=}\text{30--40}\wedge\text{income}(X){=}\text{middle}\wedge
  \text{region}(X){=}\text{Prague}\big)\ [t{:}\,25\,\%]\ \vee\ \cdots
\end{align*}

\textbf{The quantitative discriminant rule:}
\[ \forall X\ \ \text{mortgage}(X)\Leftarrow \big(\text{age}(X){=}\text{30--40}\wedge
   \text{income}(X){=}\text{high}\wedge\text{region}(X){=}\text{Prague}\big)\ [d{:}\,90\,\%] \]

\textbf{Interpreting the difference.} The last row shows why both measures are needed: the profile
\uv{20--30, low income, outside Prague} covers 10~\% of the clients with a mortgage (a
non-negligible typicality), but its d-weight is a mere 4~\% --- markedly \emph{less} than the
a-priori proportion of the target class, $200/1000=20\,\%$. This profile therefore rather
\emph{excludes} a mortgage. For the second row the d-weight is only slightly \emph{above} the
a-priori proportion (25~\% vs.\ 20~\%), so the discriminating power is negligible, and for the
third row the d-weight \emph{equals} the a-priori proportion exactly (20~\%), so the discriminating
power is \textbf{zero} --- the profile says nothing at all about the membership in the target
class, even though it covers a fifth of the clients with a mortgage.
\end{defbox}

\subsection{Measuring the interestingness of rules}

The interestingness of the derived rules is judged similarly to association rules
(Section~\ref{ssec:ar}), with an emphasis on the following measures:
...
\midrule
$t$-weight & the typicality of the profile inside the target class; a rule with a low $t$-weight
  describes a marginal phenomenon \\
$d$-weight & the discriminating power of the profile; \emph{interesting} rules are those with a
  $d$-weight markedly higher (or lower) than the a-priori class proportion \\
Lift & $\frac{d\text{-weight}}{P(C_{target})}$ --- how many times the profile increases the odds
  of the target class \\
Coverage & the fraction of all objects covered by the rule; low coverage $\Rightarrow$
  statistically unreliable \\
$\chi^2$ & the statistical significance of the relation between the profile and the class \\
Information gain & the reduction of the class entropy given the profile \\
Simplicity & the length of the rule (the number of conditions), the number of rules; Occam's
  razor \\
Unexpectedness & a contradiction with the user's model of the world (a subjective measure) \\
Actionability & one can act on its basis (a subjective measure) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{In practice:} a rule is interesting if it is at the same time (a)~\emph{comprehensible},
(b)~\emph{valid} on new data with high reliability, (c)~\emph{potentially useful} and
(d)~\emph{novel}. The first three are objectively measurable, the last requires knowledge of the
user.

\textbf{The classic trap:} a rule with a high $t$-weight but a $d$-weight equal to the a-priori
class proportion is no discovery, however impressive it may look (\uv{80~\% of the clients with a
mortgage have a bank account} --- but so do 80~\% of the clients without one).

\subsection{Summary for the examination}

\begin{itemize}
\item \textbf{Characterisation} describes the target class; \textbf{discrimination} compares it
      with contrasting classes. The difference: typicality $\neq$ discriminating power.
\item \textbf{AOI}: data collection by a query $\to$ adding \texttt{count} $\to$ \emph{attribute
      removal} (many values, no hierarchy) or \emph{attribute generalisation} (following the
      concept hierarchy) $\to$ aggregation of identical tuples $\to$ further generalisation until
      the relation is smaller than the threshold $T$ $\to$ presentation. Complexity $O(na)$ ---
      linear.
\item \textbf{Concept hierarchies}: schema, set-grouping, operation-derived, rule-based.
\item \textbf{t-weight} $=\frac{\mathrm{count}(q_a)}{\sum_i\mathrm{count}(q_i)}$ --- the typicality
      inside the target class, the sum over all tuples being 100~\%; for \emph{characteristic}
      rules ($\Rightarrow$, a necessary condition).
\item \textbf{d-weight} $=\frac{\mathrm{count}(q_a\in C_{target})}{\sum_j\mathrm{count}
      (q_a\in C_j)}$ --- the discriminating power; for \emph{discriminant} rules ($\Leftarrow$, a
      sufficient condition). A description rule combines both ($\Leftrightarrow$).
\item Be able to compute both weights from a table and to explain why a high t-weight does not imply
      a high d-weight (the comparison with the \emph{a-priori} class proportion).
\item \textbf{Interestingness}: objective (t/d-weight, lift, coverage, $\chi^2$, information gain,
      simplicity) vs.\ subjective (unexpectedness, actionability).
\end{itemize}
...
based on the \emph{modularity increase}).
\end{defbox}
