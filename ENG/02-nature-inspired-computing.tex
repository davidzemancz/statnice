...
\small
This text covers the examination topic \emph{Nature-Inspired Computing} to the extent of the
lecture courses NAIL025 (Evolutionary Algorithms~I) and NAIL086 (Evolutionary Algorithms~II),
supplemented with the standard material of the textbooks Eiben \& Smith: \emph{Introduction to
Evolutionary Computing}, Mitchell: \emph{An Introduction to Genetic Algorithms},
Michalewicz: \emph{Genetic Algorithms $+$ Data Structures $=$ Evolution Programs}
(the source, in particular, of the representations for SAT and combinatorial problems)
and Poli, Langdon \& McPhee: \emph{A Field Guide to Genetic Programming}.
It is written to be self-contained: it contains definitions, pseudocode, derivations,
numerical examples and comparison tables. Every chapter ends with a summary of the most
important points for the oral examination.
...
\midrule
Genetic algorithms
  & \ref{sec:uvod} The general scheme of an EA, \ref{sec:ga} Genetic algorithms,
    \ref{sec:reprez} Representations and operators \\
Genetic and evolutionary programming
  & \ref{sec:gp} Genetic and evolutionary programming, grammatical evolution \\
Schema theory
  & \ref{sec:schemata} Schema theory \\
Probabilistic models of the simple genetic algorithm
  & \ref{sec:vose} Probabilistic models of the SGA (Vose model, Markov chains,
    EDA) \\
Evolution strategies
  & \ref{sec:es} Evolution strategies \\
Differential evolution
  & \ref{sec:de} Differential evolution \\
Coevolution, open-ended evolution
  & \ref{sec:koevoluce} Coevolution and open-ended evolution \\
Swarm optimisation algorithms
  & \ref{sec:roje} Swarm algorithms (PSO, ACO, ABC, FA) \\
Memetic algorithms, hill climbing, simulated annealing
  & \ref{sec:lokalni} Local search and memetic algorithms \\
Applications: evolution of expert systems
  & \ref{sec:lcs} Evolution of rule-based and expert systems (LCS) \\
Applications: neuroevolution
  & \ref{sec:neuro} Neuroevolution \\
Applications: solving combinatorial problems
  & \ref{sec:kombi} Combinatorial problems and constraint handling \\
Applications: multi-objective optimisation
  & \ref{sec:moea} Multi-objective optimisation \\
(overall revision, comparison of the methods)
  & \ref{sec:zaver} Overall comparison and typical examination questions \\
\bottomrule
\end{tabular}
\end{center}
...
%=====================================================================
\section{Introduction: evolutionary algorithms and their general scheme}\label{sec:uvod}
%=====================================================================

\subsection{Placing the field}
...
\subsection{Components of an evolutionary algorithm}
...
time, chapter~\ref{sec:koevoluce}) or \emph{contextual} (dependent on the other
...
\subsection{Selection mechanisms}\label{ssec:selekce}

\subsubsection{Roulette wheel (fitness-proportionate) selection}
...
\subsubsection{Stochastic universal sampling (SUS)}
...
\subsubsection{Environmental selection, elitism, population models}
...
\subsection{Theoretical limits: No Free Lunch}
...
The practical consequence: it pays to build \textbf{domain-specific variants} of
EAs --- a suitable representation, operators that respect the structure of the
problem, and hybridisation with local heuristics. The theorem does not say that
all algorithms are equally good on \emph{real} problems; real functions form a
very small and highly structured subset of all functions.

\subsection{Historical branches of evolutionary algorithms}
...
\subsection{The simple genetic algorithm (SGA)}
...
\subsection{Mutation and inversion}
...
very effective mutation (chapter~\ref{sec:reprez}).

\subsection{Fitness scaling}\label{ssec:skalovani}
...
%=====================================================================
\section{Representation of an individual and genetic operators}\label{sec:reprez}
%=====================================================================
...
\subsection{Real-valued representation}
...
\item \textbf{SBX} (\emph{simulated binary crossover}): it imitates the
  ...
  are distributed symmetrically around the parents with a density controlled by
  the parameter $\eta$. The standard operator in NSGA-II.
\end{itemize}
...
\subsubsection{OX --- order crossover}
...
\subsubsection{CX --- cyclic crossover}
...
\subsubsection{ER --- edge recombination}
...
\subsubsection{Mutation of permutations}
...
Trees (chapter~\ref{sec:gp}), graphs and DAGs (Cartesian GP, neuroevolution),
matrices (schedules, the TSP as a predecessor matrix), finite automata (EP),
rule lists (LCS, chapter~\ref{sec:lcs}), artificial-life agents.
...
%=====================================================================
\section{Schema theory}\label{sec:schemata}
%=====================================================================
...
\subsection{A schema, its order and defining length}
...
\subsection{Holland's schema theorem}
...
\subsection{The building blocks hypothesis}
...
\subsection{Implicit parallelism and the multi-armed bandit}
...
\subsection{Criticism of schema theory}

\subsubsection{When a GA fails --- deceptive problems}

Consider a function in which the schemata $(111{*}{*}{*}{*}{*}{*}{*})$ and
$({*}{*}{*}{*}{*}{*}{*}{*}11)$ are above average, but
\[
...
\subsubsection{Static average fitness --- the static BBH}
...
\subsubsection{Summary of the weaknesses}
...
%=====================================================================
\section{Probabilistic models of the simple GA}\label{sec:vose}
%=====================================================================
...
\subsection{Formalisation of the population}
...
\subsection{The operator $\mathcal{G}$}
...
\subsection{Results for infinite populations}
...
\subsection{Finite populations: a Markov chain}
...
\subsection{Estimation of distribution algorithms (EDA)}\label{ssec:eda}

Vose's model shows that the SGA is in fact a machine for transforming a
probability distribution over the space of strings. \emph{Estimation of
distribution algorithms} (EDA) take this observation literally: they throw away
both crossover and mutation and work with the model directly.

\begin{defbox}[EDA --- the general scheme]
Instead of operators, an \textbf{explicit probabilistic model}
$p_\theta(x)$ over the solution space is maintained. Iteration:
(1)~sample a population from $p_\theta$, (2)~evaluate it and select the better
part, (3)~\emph{re-learn} the model $\theta$ on the selected individuals,
(4)~repeat. The model $\theta$ is the only information carried between
generations.
\end{defbox}

The algorithms differ in how rich the dependences between the genes are that the
model can capture:
...
\toprule
\textbf{model} & \textbf{algorithm} & \textbf{note} \\
\midrule
no dependences & PBIL, UMDA, cGA &
  a vector $p = (p_1,\dots,p_m)$ of probabilities of a one at each position \\
pairwise dependences & MIMIC, BMDA &
  a chain or tree of dependences between the genes \\
general dependences & BOA, hBOA, EBNA &
  a Bayesian network learned from the selected individuals \\
continuous domains & EMNA, CMA-ES &
  a Gaussian distribution, resp.\ its covariance matrix \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{UMDA} (univariate marginal distribution algorithm) simply computes,
on the selected individuals, the relative frequency of a one at each position
and samples the whole new population from these marginals.

\paragraph{PBIL} (population-based incremental learning) does not store that
vector anew, but shifts it towards the best individual with a learning rate
$\alpha$:
\[
p_i \;\gets\; (1-\alpha)\,p_i \;+\; \alpha\, x_i^{\mathrm{best}},
\qquad \alpha \approx 0.1 ,
\]
plus a small random perturbation as an analogue of mutation. The population can
then be regarded as a mere auxiliary sample --- the \uv{gene pool} is the vector
$p$.

\textbf{Example.} Let $p = (0.5;\,0.5;\,0.5)$ and let the best individual be
$x^{\mathrm{best}} = (1,0,1)$, $\alpha = 0.2$. Then
$p \gets (0.8\cdot 0.5 + 0.2;\ 0.8\cdot 0.5;\ 0.8\cdot 0.5 + 0.2)
= (0.6;\,0.4;\,0.6)$ --- the model has leant towards the pattern $101$.

\paragraph{Why it works (and when it does not).} Independent marginals
correspond exactly to what uniform crossover does on an already converged
population, but without the disruptiveness. As soon as there is
\emph{epistasis} (interaction) between the genes, however, a univariate model
does not capture it and the algorithm fails --- hence BOA, which learns the
structure of the dependences (\emph{linkage}). EDAs thus elegantly solve the
problem for the sake of which Holland once proposed the inversion operator:
where the cooperating genes should lie is something the algorithm learns by
itself.

\subsection{Other theoretical tools (an overview)}

\begin{itemize}
\item \textbf{Runtime analysis}: the asymptotic number of fitness evaluations
  for simple algorithms and functions. Classically, the $(1{+}1)$-EA needs
  $\Theta(m \log m)$ evaluations on the OneMax function, $\Theta(m^2)$ on the
  LeadingOnes function, and exponentially many on a \uv{needle in a haystack}.
  The tools: drift analysis, the fitness levels method.
\item \textbf{Fitness landscape analysis}: the fitness correlation of
  neighbours, the number of local optima, epistasis (gene interaction),
  NK landscapes.
\item \textbf{Exact schema theorems} (Stephens \& Waters): equalities (not
  inequalities) that describe the \emph{constructive} effect of crossover as
  well --- exactly what Holland's theorem lacks.
\end{itemize}
...
\subsection{Motivation and basic features}
...
\subsection{$(1+1)$-ES and the 1/5 rule}
...
\subsection{Self-adaptation of the strategy parameters}
...
\subsection{Recombination in ES}
...
\emph{Covariance Matrix Adaptation ES} (Hansen, Ostermeier) is nowadays the
reference algorithm for continuous black-box optimisation. Instead of
self-adapting the individual $\sigma$ values in the genome, it maintains
\textbf{a single distribution for the whole population} and learns its
covariance matrix from the history of successful steps.

\begin{defbox}[The principle of CMA-ES]
The state of the algorithm is
$\theta = (\vec m, \sigma, \mathbf{C}, \vec p_\sigma, \vec p_c)$ and the
sampling distribution is
\[
p_\theta(\vec x) \;=\; \mathcal{N}\big(\vec x \mid \vec m, \; \sigma^2 \mathbf{C}\big),
\]
where $\vec m$ is the centre (the centroid of the best individuals), $\sigma$
the global step length, $\mathbf{C}$ the covariance matrix determining the shape
and orientation of the cloud of samples, and $\vec p_\sigma, \vec p_c$ are the
\emph{evolution paths} --- exponentially smoothed records of the direction in
which the centre has been moving in the last iterations.
\end{defbox}

The iteration:
\begin{enumerate}
\item Sample $\lambda$ points $\vec x_i = \vec m + \sigma\,\mathcal{N}(\vec 0, \mathbf{C})$.
\item Evaluate them and select the $\mu$ best ones (\emph{rank-based} --- CMA-ES
  uses only the ordering, so it is invariant with respect to monotone
  transformations of fitness).
\item The new centre $\vec m' = \sum_{i=1}^{\mu} w_i \vec x_{i:\lambda}$
  (a weighted average of the best ones).
\item Update the evolution paths in proportion to the shift
  $\vec m' - \vec m$.
\item \textbf{Step-length control:} if $\|\vec p_\sigma\|$ is larger than would
  correspond to a random walk, the steps are going in a consistent direction
  $\Rightarrow$ increase $\sigma$; if the steps are opposing each other,
  decrease $\sigma$.
\item \textbf{Adaptation of $\mathbf{C}$:} a \emph{rank-1 update} according to
  $\vec p_c \vec p_c^{\!\top}$ (which supports the direction of the progress made
  so far) and a \emph{rank-$\mu$ update} according to the empirical covariance
  of the selected points.
\end{enumerate}

The result is an algorithm that adapts automatically to the anisotropy and the
correlations of the problem --- in essence, it learns an approximation of the
inverse Hessian matrix without a gradient (\emph{invariance with respect to
linear transformations of the space}). It has very few tunable parameters (the
defaults depend on $n$ only).

\paragraph{Classification: the general principle of stochastic search.} CMA-ES
is a special case of a general template: the algorithm maintains a distribution
$p_\theta(\vec x)$, in each iteration samples $n$ points from it, evaluates them
and updates $\theta$ according to the data. The parameter $\theta$ is the only
information carried between iterations. Depending on the choice of $\theta$ we
obtain: an EA ($\theta$ = the parent population), an ES ($\theta$ = the mean and
the variance of a Gaussian), an EDA ($\theta$ = the parameters of a model,
e.g.\ of a Bayesian network), simulated annealing ($\theta$ = the current point
and the temperature).
...
\subsection{The DE/rand/1/bin algorithm}
...
\subsection{A numerical example of one step}
...
\subsection{Comparison of DE with ES and GA}
...
%=====================================================================
\section{Genetic and evolutionary programming, grammatical evolution}\label{sec:gp}
%=====================================================================
...
Three competing (and mutually compatible) theories explain it:
\begin{itemize}
\item \textbf{Replication accuracy} --- introns protect an individual against
  destructive crossover (a larger share of neutral cut points), so larger
  individuals pass their fitness on more reliably; the introns therefore
  \uv{hitchhike} along with the good code.
\item \textbf{Removal bias} --- the removed subtree must be small if the
  individual's fitness is not to get worse, but the inserted subtree may be
  arbitrarily large; the mean size therefore grows systematically.
\item \textbf{Crossover bias} --- random subtree crossover generates a limiting
  distribution of sizes with a heavy tail and with many \emph{very small}
  individuals; those have poor fitness, selection eliminates them, and the
  average size of the population thereby grows.
\end{itemize}
More generally: in the space of programs there are far more large programs with
the same behaviour than small ones, so neutral drift by itself pushes towards
growth. In nature growth runs into physical and energetic limits; in GP bloat
has to be fought explicitly:
\begin{itemize}
\item \textbf{Hard limits} on the depth or the number of nodes (an offspring
  exceeding the limit is discarded or replaced by the parent).
\item \textbf{Parsimony pressure} --- a penalty term in the fitness
  $f' = f - \alpha \cdot \text{size}$; or a \emph{lexicographic} variant: with
  equal fitness the smaller individual wins.
\item \textbf{Anti-bloat operators} --- size-fair crossover and mutation, hoist,
  shrink.
\item \textbf{A multi-objective approach} --- size as a second criterion
  (chapter~\ref{sec:moea}).
\end{itemize}

\subsection{ADFs and typed GP}

\begin{defbox}[ADF (automatically defined functions)]
Subprograms evolved together with the main program. An individual consists of a
\texttt{main} branch and one or more ADF branches; a call of an ADF behaves in
the main program as a new non-terminal of the given arity. The genetic operators
work on the branches \textbf{separately} (main is crossed with main, ADF$_1$
with ADF$_1$). ADFs bring \textbf{modularity} and the ability to capture
symmetries and reusable subproblems; without them GP scales poorly.
\end{defbox}

Extensions: automatically defined loops, iterations, recursion, memory. An
alternative is the \emph{coevolution} of two populations --- of main programs
and of subprograms (chapter~\ref{sec:koevoluce}).

\paragraph{Enforcing structure.}
\begin{itemize}
\item \textbf{Strongly typed GP}: every node has a type of its inputs and of its
  output; the operators may create only type-correct trees. It reduces the
  search space and makes mixed domains possible (numbers, Boolean values,
  vectors). Type hierarchies and polymorphism exist as well.
\item \textbf{Grammar-based GP}: the admissible shapes of the trees are
  described by a context-free grammar, which is used both at initialisation and
  as a guide for the operators.
\end{itemize}

\subsection{Linear and graph GP}
...
\paragraph{Cartesian GP (CGP).} An elegant way around this: a DAG is represented
...
The genotype is a \textbf{linear list of integers} (of variable length), the
phenotype is a program derived from a context-free grammar. A number from the
genome determines which rule is used to expand the leftmost non-terminal:
\[
\text{rule} \;=\; (\text{gene} \bmod \text{the number of rules for this non-terminal}).
\]
\end{defbox}

The procedure: genotype $\to$ a sequence of applied rules $\to$ a derivation
tree $\to$ (by reduction) a syntax tree = the phenotype.

Properties:
\begin{itemize}
\item The use of the modulo operation makes the encoding \textbf{robust} ---
  every genome is interpretable.
\item The genome is linear $\Rightarrow$ \textbf{the standard GA operators} can
  be used, which is the main practical advantage.
\item The grammar guarantees syntactic correctness and makes it easy to impose
  domain knowledge or types.
\item Technical problems: the derivation need not terminate. This is handled by
  \emph{wrapping} (once the genome is exhausted, it is read again from the
  beginning, with a limit on the number of wraps), by low fitness for
  inadmissible individuals, or by a repair (once a limit is exceeded, the rules
  that expand non-terminals are removed from the grammar).
\item Disadvantage: the operators are not \uv{local} --- a small change of the
  genotype (especially at the beginning) may change the phenotype completely
  (the \emph{ripple effect}).
\item \emph{Grammatical swarm}: PSO is used for the search instead of a GA.
\end{itemize}
...
\subsection{Is crossover good for anything? The headless chicken experiment}
...
%=====================================================================
\section{Coevolution and open-ended evolution}\label{sec:koevoluce}
%=====================================================================

\subsection{Contextual fitness}
...
\subsection{Pathologies of coevolution}
...
chapter~\ref{sec:neuro}), meta-Lamarckian memetic algorithms (a population of
memes and a population of solutions, chapter~\ref{sec:lokalni}).

\subsection{The evolution of cooperation and the prisoner's dilemma}
...
\subsection{Diversity, species and niches}
...
\item \textbf{Explicit species} by means of tag bits, or according to the
  structural similarity of the genomes (NEAT, chapter~\ref{sec:neuro}); mating
...
\subsection{Dynamic fitness landscapes}
...
An evolutionary process that \textbf{has no fixed goal} and that keeps producing
new complexity and new \uv{kinds} of behaviour without converging. The model is
the terrestrial biosphere: it has no objective function, and yet it permanently
generates novelty. In artificial life one watches whether a system exhibits a
sustained growth of complexity, the emergence of new classes of entities
(parasites, hyperparasites), ecological relationships and a dynamics that does
not come to a halt.
\end{defbox}

\paragraph{The hallmarks of open-ended evolution.} In order to be able to decide
about a system at all, \emph{hallmarks} of open-endedness are formulated:
\begin{itemize}
\item \textbf{the sustained production of novelty} --- new behaviour appears
  even after an arbitrarily long run, the system does not settle down;
\item \textbf{the unbounded growth of the complexity} of the individuals and of
  their mutual relationships;
\item \textbf{the emergence of new classes of entities} and of new levels of
  organisation (in biology the \emph{major transitions}: replicator $\to$ cell
  $\to$ multicellularity $\to$ society);
\item \textbf{the evolution of evolvability} --- the very way in which the
  system generates novelty changes as well (the representation, the operators,
  the genotype--phenotype mapping).
\end{itemize}

\paragraph{The systems and why they get stuck.}
\emph{Tierra} (Ray) and \emph{Avida} --- self-copying machine-code programs in
which parasites and hyperparasites emerged; \emph{Polyworld} (Yaeger) --- agents
with neural networks in a 2D world; \emph{ECHO} (Holland) --- agents with
\uv{tags} and resources modelling ecological and economic relationships.
Practically every one of them reaches, after some time, a state in which nothing
qualitatively new appears any more: the environment is finite, the space of
possible \uv{inventions} is exhausted and the dynamics freezes. The main
hypotheses as to why: the lack of a sufficiently rich and self-extending
environment, the lack of a genuinely open representation, and the lack of a
mechanism that would continually raise the demands placed on the individuals.
Achieving genuinely open-ended evolution remains an open research problem.

\paragraph{Generative (morphogenetic) representations.} Closely related to
open-ended evolution is the question of how to encode unboundedly growing
complexity at all. The answer are \emph{indirect} encodings, in which the
genotype is not a description of the result but \textbf{a recipe for growing the
result}:
\begin{itemize}
\item \textbf{L-systems} (Lindenmayer) --- rewriting grammars generating
  branching structures (plants, vascular systems, robot morphologies);
\item \textbf{cellular automata} --- local rules generating global patterns; the
  classical model of emergence;
\item \textbf{cellular encoding} (Gruau) and \textbf{CPPN/HyperNEAT}
  (chapter~\ref{sec:neuro}) --- the same principles applied to neural networks;
\item \textbf{developmental GP} (chapter~\ref{sec:gp}) --- a tree-program that
  builds a circuit or a graph.
\end{itemize}
Their common contribution: a small genome, a large phenotype, and a natural
expression of symmetries and of repeated motifs --- exactly what nature does and
what direct encodings cannot do.

\paragraph{Novelty search.} A radical idea (Lehman \& Stanley, 2011:
\emph{Abandoning Objectives}): in deceptive problems the objective function is
misleading --- it leads into local optima and there is no continuous trajectory
to the global optimum. Therefore \textbf{let us throw the goal away and reward
novelty alone}.

\begin{defbox}[Sparseness and novelty search]
We define the \emph{behaviour space} --- the space of phenotypic manifestations
(e.g.\ the final position of a robot in a maze). The novelty of an individual
$x$ is measured by the sparseness of its neighbourhood:
\[
\rho(x) \;=\; \frac{1}{k}\sum_{i=1}^{k} \mathrm{dist}\big(x, \mu_i\big),
\]
where the $\mu_i$ are the $k$ nearest neighbours of $x$ in the current
population \textbf{and in a permanent archive}. Individuals with $\rho$ above a
threshold are added to the archive. Fitness = $\rho$; the original objective
function \emph{is not used at all}.
\end{defbox}

On problems of the \uv{robot in a maze} type, novelty search tends to be more
successful than goal-directed optimisation, because it systematically traverses
the reachable behaviours. It is combined with NEAT. The follow-up direction
\textbf{quality-diversity} (\emph{novelty search with local competition},
MAP-Elites) looks for an archive of solutions that are both diverse \emph{and}
of good quality within their niche; MAP-Elites maintains a grid of cells indexed
by behaviour descriptors and, in each cell, the best solution found. A further
direction are algorithms that generate their own problems (\emph{minimal
criterion coevolution}, POET).

\subsection{Interactive evolution}

\begin{defbox}[Interactive evolutionary computation (IEC)]
The fitness function is replaced by a \textbf{human being}: the user is
presented with a population of candidates and picks the ones they like or find
promising. It is used wherever we cannot write down the quality criterion ---
aesthetics, design, music, taste, ergonomics, but also a witness's identikit.
\end{defbox}

The classical examples are Dawkins's \emph{Biomorphs}, Sims's evolved images
and, above all, \textbf{Picbreeder} and \textbf{EndlessForms} (the evolution of
pictures and of 3D shapes, respectively, by means of CPPNs,
chapter~\ref{sec:neuro}), where the users moreover build on other people's
half-finished lineages --- so this is collective, open-ended and non-goal-driven
evolution. It was precisely the experience from Picbreeder (the most interesting
objects were never produced by anyone searching for them deliberately) that
directly motivated novelty search.

\textbf{Limitations:} a human being is incomparably slower and more expensive
than a computation $\Rightarrow$ small populations are necessary (typically
9--20 individuals per screen), few generations, and the user's fatigue leads to
inconsistent evaluation. The remedies: evaluating only the ranking instead of
numbers, interleaving the human evaluation with a learned \emph{surrogate}
model, or using IEC only to steer an otherwise automatic evolution from time to
time.
...
%=====================================================================
\section{Swarm optimisation algorithms}\label{sec:roje}
%=====================================================================
...
%=====================================================================
\section{Local search and memetic algorithms}\label{sec:lokalni}
%=====================================================================
...
\paragraph{Nelder--Mead (the downhill simplex).} For continuous domains the
simplex method is often used as a gradient-free local method: $n+1$ points in
$\mathbb{R}^n$ (a simplex) are maintained and the worst of them is replaced by
means of the operations \emph{reflection}, \emph{expansion}, \emph{contraction}
and \emph{shrinking} of the simplex. It is the most common \uv{meme} in memetic
algorithms for real-valued parameters and, together with hill climbing and
simulated annealing, it belongs to the standard equipment of black-box
optimisation.
...
(chapter~\ref{ssec:selekce}) carries the same idea over into selection.
...
\item Hill climbing: steepest ascent / first improvement / stochastic / with
  ...
  For continuous domains additionally Nelder--Mead (reflection, expansion,
  contraction, shrinking of the simplex).
...
%=====================================================================
\section{Evolution of rule-based and expert systems}\label{sec:lcs}
%=====================================================================
...
\subsection{The Pittsburgh approach and the GIL system}
...
\subsection{A reminder: classification metrics}

From the confusion matrix (TP, FP, FN, TN):
\begin{gather*}
\text{accuracy} = \frac{TP+TN}{TP+TN+FP+FN}, \\[3pt]
\text{sensitivity} = \frac{TP}{TP+FN}, \qquad
\text{specificity} = \frac{TN}{TN+FP}.
\end{gather*}
The fitness of a rule-based system is usually a combination of accuracy,
coverage and simplicity (the number of rules) --- so it is naturally a
...
\subsection{HyperNEAT and CPPNs}

\begin{defbox}[HyperNEAT]
An extension of NEAT based on two ideas:
\textbf{(1) The substrate} --- the weights of the network are not encoded as a
vector of numbers, but \emph{are generated by a function} $S$ which receives the
coordinates of two neurons in the plane and returns the weight of their
connection: $S : \mathbb{R}^4 \to \mathbb{R}$
(hence \uv{hyper} --- it maps a hypercube into $\mathbb{R}$;
in a 3D arrangement it is $S: \mathbb{R}^6 \to \mathbb{R}$).
\textbf{(2) A CPPN} (compositional pattern-producing network) --- the function
$S$ is represented by a network in the form of a DAG whose nodes compute various
functions from a dictionary (sigmoid, Gaussian, sine, cosine, polynomials,
absolute value). This CPPN is evolved by NEAT itself.
\end{defbox}

The benefit: representing the weights by a function makes it possible to express
the \textbf{symmetries, repetitions and regularities} of the problem and to
generate networks with millions of connections from a small genome (the geometry
of the problem is encoded in the coordinates of the substrate). Variants: the
evolution of deep structures, the evolution of the neuron positions as well
(ES-HyperNEAT), 3D substrates. Demonstrations of the aesthetic potential of
CPPNs: Picbreeder, EndlessForms.

\subsection{Neuroevolution in the era of deep networks}

\paragraph{NAS (neural architecture search).} The search for the architecture of
deep networks is described along three axes:
\begin{itemize}
\item \textbf{The search space}: linear (a sequence of layers --- the type of
  the layer and its hyperparameters: the number of units, the kernel size, the
  number of filters, the stride, the type of pooling; the chromosome is of
  variable length and conditional), a DAG of modules (modern architectures have
  \emph{skip} connections; one searches for \emph{cells} --- \uv{normal} ones
  preserving the dimensions and \uv{reduction} ones shrinking them --- which are
  then stacked), or a subarchitecture of one large \uv{supernetwork}.
\item \textbf{The search strategy}: evolutionary algorithms, random search
  (a surprisingly strong baseline), Bayesian optimisation, reinforcement
  learning, gradient methods (DARTS).
\item \textbf{Performance estimation}: shortened training, lower resolution,
  smaller networks, weight sharing, learning-curve predictors; single- or
  multi-objective (accuracy vs.\ latency, the number of parameters).
\end{itemize}

\paragraph{Concrete systems.}
\begin{itemize}
\item \textbf{ENAS} (efficient NAS): the space is a single large DAG and the
  search means selecting a subgraph; \emph{weight sharing} among the candidates
  removes the need to train every model from scratch (a speed-up by orders of
  magnitude).
\item \textbf{DeepNEAT}: a node of the chromosome is not a neuron but a whole
  DNN layer; repairs are necessary so that the dimensions of the inputs and the
  outputs fit together.
\item \textbf{CoDeepNEAT}: the coevolution of two populations --- of
  \emph{modules} (small DNNs) and of \emph{blueprints} (graphs whose nodes refer
  to modules). It has also been applied to LSTMs with special mutations (a
  change of the internal structure of the cell, the search for an arrangement of
  the units in a layer).
\item \textbf{Hill-climbing NAS} (Elsken, Metzen, Hutter 2018): start with a
  small pre-trained network and, in a hill-climbing loop, apply network
  \emph{morphisms} (deepen --- add a ConvBlock; widen --- increase the number of
  channels; add skip connections), then briefly retrain the offspring. An
  illustration of the memetic approach in NAS.
\item \textbf{Deep neuroevolution / ES for RL} (OpenAI 2017; Salimans et al.,
  Miikkulainen and Clune): a simple ES \emph{with mutations only} over millions
  of weights of deep networks for MuJoCo and Atari tasks. An ES can be
  understood as an estimate of the gradient of a smoothed fitness; the algorithm
  is trivially parallelisable (the nodes exchange only the seeds of the random
  number generator) and it beat A3C roughly 10$\times$ in wall-clock time. For
  supervised learning (image classification) backpropagation remains
  considerably faster.
\end{itemize}
...
%=====================================================================
\section{Combinatorial problems and constraint handling}\label{sec:kombi}
%=====================================================================
...
\subsection{Three general strategies for constraints}
...
(chapter~\ref{sec:moea}) --- the result is then a whole Pareto front of
...
\textbf{The task.} A formula $f: \mathbb{B}^n \to \mathbb{B}$ in CNF (a
conjunction of clauses, a clause = a disjunction of literals); find an
assignment $x$ with $f(x)=1$. 2-SAT is in P, 3-SAT and above is NP-complete. The
paradigmatic NP-complete problem.

\paragraph{Representations.}
\begin{itemize}
\item \textbf{A bit string} --- the individual is directly an assignment of the
  variables.
\item \textbf{Real-valued} --- the variable $x_j$ is replaced by a real $y_j$,
  true corresponds to $y = 1$, false to $y = -1$, the literals are rewritten as
  $x \mapsto (1-y)^2$ and $\neg x \mapsto (1+y)^2$, and the whole expression is
  \emph{minimised}; the result is rounded at the end (negative values to $-1$,
  positive ones to $1$). The value of a literal is therefore a \emph{penalty}: a
  satisfied literal gives 0, an unsatisfied one gives 4. That is why a
  \textbf{disjunction} (a clause) is rewritten as a \textbf{product} and a
  \textbf{conjunction} (the whole formula) as a \textbf{sum}: a clause is
  satisfied if one literal suffices --- and a product is zero if one of its
  factors is zero; the formula is satisfied if all the clauses are satisfied ---
  and a sum is zero if all its summands are zero. (This is the pair most often
  swapped around; always verify it on $(x_1 \vee x_2)$ with $y_1 = 1$,
  $y_2 = -1$: the product gives $0 \cdot 4 = 0$, \uv{satisfied}, whereas the sum
  $0 + 4 = 4$ would incorrectly declare the clause unsatisfied.)
\item \textbf{Clause-based} --- for every clause the admissible assignments of
  its variables are found; the individual is a vector of assignments for all the
  clauses (of length $m \cdot k$ for $k$-SAT with $m$ clauses). A special
  fitness resolving the global inconsistencies is necessary.
\item \textbf{Path-based} --- the clauses are traversed and in each of them at
  least one consistent assignment is chosen; the individual does not determine
  all the variables, so it represents a set of solutions. Again a special
  fitness is needed.
\end{itemize}

\paragraph{Fitness.} $f$ itself is unusable (it takes only the values 0/1, so
the landscape is a \uv{needle in a haystack}). The standard is the
\textbf{number of satisfied clauses}; the improvements: weighting the
problematic clauses (the weights are updated after a few iterations) and a
\emph{refining function} --- a regularisation term $\alpha\,r(x)$ is added to
the fitness in order to distinguish individuals with the same number of
satisfied clauses (and thus to help off the plateaux). The weights $v_j$ capture
the tendency of the variable $x_j$ to take the value 1 or 0. The classical local
heuristic for SAT is \textbf{WalkSAT/WSAT} --- it evaluates a solution by the
number of satisfied clauses and cleverly chooses the direction of the local step
(a combination of a greedy and a random choice); the combination EA + WSAT is a
typical memetic solution.

\subsection{Scheduling and production planning}

\paragraph{A school timetable.} An individual is a timetable in the form of a
matrix (rows = teachers, columns = classes, values = subject codes). Mutation
shuffles the subjects, crossover exchanges the better rows between individuals.
The fitness combines the quality of the individual rows (the teacher's
satisfaction) and further \emph{soft} criteria (gaps, uniformity).
\textbf{Hard constraints} (a teacher cannot teach two classes at once, the
availability of the rooms) have to be respected \emph{directly in the
operators} --- otherwise far too many infeasible individuals arise.

\paragraph{Job shop scheduling.} The products $o_1 \ldots o_N$ consist of parts
$p_1 \ldots p_K$; for each part there are several production plans on the
machines $m_1 \ldots m_M$, and resetting a machine between products costs time.
The fitness = the total production time (the \emph{makespan}).
\textbf{The encoding is critical:}
\begin{itemize}
\item \emph{Permutation-based}: the individual is only the order of the
  products, and the plans of the parts are determined by a decoder. Simple, and
  the crossovers from the TSP can be used --- but it turns out to be not very
  effective: the difficult part is solved by the decoder and the TSP operators
  are not appropriate here.
\item \emph{Direct}: the individual is a complete plan; specialised and complex
  evolutionary operators are necessary. Harder to design, but more powerful.
\end{itemize}
...
%=====================================================================
\section{Multi-objective optimisation}\label{sec:moea}
%=====================================================================

\subsection{Dominance and the Pareto front}
...
\subsection{Scalarisation --- the simple solutions}
...
\subsection{The historical algorithms}
...
\subsection{Other important algorithms}

\paragraph{SPEA2 (Zitzler, 2001).} It uses an external \textbf{archive} of
non-dominated solutions. The fitness has two parts: the \emph{raw fitness} of an
individual is the sum of the \emph{strengths} of all the individuals that
dominate it, where the strength = the number of individuals the given individual
dominates (smaller is better, the non-dominated ones have 0); to this a
\emph{density} derived from the distance to the $\sigma$-th nearest neighbour is
added ($\sigma = \sqrt{N + \bar N}$). The truncation of the archive
progressively removes the individuals with the nearest neighbour, which
guarantees that the extreme points are preserved.

\paragraph{MOEA/D (Zhang \& Li, 2007).} A decomposition approach: the problem is
decomposed into $\mu$ \emph{scalar} subproblems with evenly distributed weight
vectors $\lambda^{(1)},\dots,\lambda^{(\mu)}$ (Chebyshev scalarisation with
respect to a reference point $z^*$). Every individual of the population
corresponds to one subproblem and cooperates only with its \emph{neighbourhood}
$B(i)$ (the subproblems with the nearest weight vectors). In each iteration two
parents are selected from $B(i)$ for every $i$, an offspring $y$ is created
(possibly improved by a local search), and $y$ replaces those solutions in
$B(i)$ for whose scalarised function $g(\cdot\,|\,\lambda^{(j)}, z^*)$ it is
better; $z^*$ is updated along the way. Advantages: low complexity, good
scalability to many criteria, a naturally even coverage of the front.

\paragraph{SMS-EMOA / indicator-based algorithms.} Selection directly maximises
a quality \emph{indicator} --- typically the hypervolume (the $S$-metric). The
algorithm is steady-state: one offspring is created, merged with the population,
and the individual (from the worst front) whose removal decreases the
hypervolume the least (the smallest \emph{hypervolume contribution}) is
eliminated. From a theoretical point of view this is the best-founded approach
(maximising the hypervolume implies Pareto optimality), but computing the
hypervolume is exponentially expensive in the number of criteria.

\subsection{Metrics of approximation quality}

\begin{defbox}[Hypervolume (the $S$-metric)]
The volume (in the space of criteria) of the region dominated by the front found
and bounded by a \emph{reference point} $r$ (chosen so that it is dominated by
all the solutions):
\[
HV(A, r) = \mathrm{Vol}\Big(\bigcup_{a \in A} [f_1(a), r_1] \times \cdots
\times [f_k(a), r_k]\Big).
\]
The only common metric that is \emph{strictly monotone with respect to
dominance} (a better front always has a higher hypervolume) and that does not
require knowledge of the true Pareto front.
\end{defbox}

\textbf{A numerical example.} The front $\{(2,7), (4,5), (8,2)\}$, the reference
point $r = (10,10)$. We split it into vertical strips (sorting by $f_1$ in
decreasing order):
\[
...
(2,7): (4-2)(10-7) = 6 .
\]
In total $HV = 42$. If we add the dominated point $(5,8)$, the value does not
change; if we add a non-dominated point, it grows.

\paragraph{Other metrics.}
\emph{GD} (generational distance) --- the average distance of the points found
from the true front (it measures convergence);
\emph{IGD} (inverted GD) --- the average distance of the points of the true
front from the approximation found (it measures both convergence and coverage;
both require knowledge of the true front);
\emph{spread/spacing} --- the evenness of the distribution;
the \emph{$\varepsilon$-indicator} --- the smallest $\varepsilon$ by which the
front $A$ has to be shifted in order to dominate $B$.

\subsection{An overall comparison of MOEAs}
...
%=====================================================================
\section{Overall comparison and typical examination questions}\label{sec:zaver}
%=====================================================================
