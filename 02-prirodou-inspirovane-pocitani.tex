...
doslova: zahodí křížení i~mutaci a~udržují \textbf{explicitní model} $p_\theta(x)$.
Iterace: navzorkuj z~$p_\theta$ populaci, vyber lepší část, \emph{přeuč} na ní model,
opakuj. Model $\theta$ je jediná informace přenášená mezi generacemi.

Podle bohatosti zachycených závislostí: \textbf{bez závislostí} (UMDA, PBIL),
\textbf{párové} (MIMIC), \textbf{obecné} (BOA --- Bayesovská síť), \textbf{spojité}
(CMA-ES). PBIL posouvá vektor pravděpodobností k~nejlepšímu jedinci:
$p_i \gets (1-\alpha)\,p_i + \alpha\, x_i^{\mathrm{best}}$.
\end{defbox}

Univariátní model selže, jakmile mezi geny existuje \emph{epistáze} --- proto BOA, který se
strukturu závislostí učí. Tím EDA řeší problém, kvůli němuž Holland navrhoval inverzi:
kde mají spolupracující geny ležet, se algoritmus naučí sám.
...
\item Implicitní paralelismus ($\sim n^3$ užitečně zpracovaných schémat)
  a~analogie s~banditou: schémata řádu $k$ soupeří o~tytéž $k$ pevných pozic,
  hrají tedy hru s~$2^k$ pákami; analytické řešení alokuje vítězi exponenciálně
...
Slidy EVA~II tuto myšlenku formulují u~CMA-ES, ale platí pro celý okruh a~je to nejlepší
možné zahájení kterékoli odpovědi.

\begin{defbox}[Stochastické black-box hledání]
\textbf{Black-box optimalizace}: minimalizujeme $f : \mathbb{R}^n \to \mathbb{R}$, přičemž
jedinou dostupnou informací o~$f$ jsou \emph{její hodnoty ve zvolených bodech} (žádný gradient,
žádný analytický popis) a~cenou hledání je \emph{počet vyhodnocení}~$f$.

Všechny probírané metody mají pak tentýž skelet: algoritmus udržuje \textbf{rozdělení
pravděpodobnosti} $p_\theta(x)$ nad prostorem řešení a~opakuje
\[
...
\]
Parametr $\theta$ je \emph{jediná} informace přenášená mezi iteracemi: kóduje, co se algoritmus
dosud naučil, a~určuje, kde bude hledat dál. Jednotlivé metody se liší jen tím, \emph{co je}
$\theta$ a~jak zní aktualizační heuristika~$h$.
\end{defbox}
...
Z~téhle perspektivy je EDA (\S\ref{ssec:eda}) jen důsledné domyšlení: když je populace stejně
jen implicitní model, proč ho nemodelovat rovnou explicitně. A~naopak --- pochopení, že GA je
transformace rozdělení, je přesně obsahem Voseho modelu (\S\ref{ssec:vose}).
